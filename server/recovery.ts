...
const planItemSchema = z.object({
  ...
  monthlyAmount: moneySchema.refine((amount) => amount > 0, "O valor mensal do plano deve ser maior que zero."),
  dueDay: z.coerce.number().int().min(1).max(31).optional().nullable(),
...
const reviewPatchSchema = z.object({ status: itemStatusSchema, summary: optionalText(4000), balanceSnapshot: signedMoneySchema.nullable(), snoozedUntil: dateSchema.nullable(), actions: z.array(actionInputSchema).max(100) }).partial();
...
async function replacePlanItems(client: PoolClient, userId: string, planId: string, items: z.infer<typeof planItemSchema>[]) {
  await client.query("DELETE FROM recovery_plan_items WHERE plan_id=$1 AND user_id=$2", [planId, userId]);
  for (const item of items) {
    const debt = await client.query("SELECT id FROM debts WHERE id=$1 AND user_id=$2", [item.debtId, userId]);
    if (!debt.rowCount) throw new Error(`Dívida ${item.debtId} não encontrada para este usuário.`);
    let acceptedNegotiationId = item.acceptedNegotiationId ?? null;
    if (acceptedNegotiationId) {
      const negotiation = await client.query("SELECT id FROM debt_negotiations WHERE id=$1 AND debt_id=$2 AND user_id=$3 AND status='accepted'", [acceptedNegotiationId, item.debtId, userId]);
      if (!negotiation.rowCount) throw new Error("A negociação vinculada precisa pertencer à dívida e estar aceita.");
    } else {
      const accepted = await client.query("SELECT id FROM debt_negotiations WHERE debt_id=$1 AND user_id=$2 AND status='accepted' ORDER BY decided_at DESC NULLS LAST, created_at DESC LIMIT 1", [item.debtId, userId]);
      acceptedNegotiationId = accepted.rows[0]?.id ?? null;
    }
    await client.query("INSERT INTO recovery_plan_items (id,user_id,plan_id,debt_id,accepted_negotiation_id,monthly_amount_cents,due_day,sequence,notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)", [randomUUID(), userId, planId, item.debtId, acceptedNegotiationId, moneyToCents(item.monthlyAmount), item.dueDay ?? null, item.sequence, item.notes ?? null]);
  }
}
...
router.patch("/weekly-reviews/:id", asyncRoute(async (request, response) => {
  const parsed = reviewPatchSchema.safeParse(request.body);
  if (!parsed.success || !validDate(parsed.data?.snoozedUntil) || parsed.data?.actions?.some((action) => !validDate(action.dueDate))) {
    return validationError(response, "Dados da revisão semanal inválidos.", parsed.success ? undefined : parsed.error.flatten());
  }
  const data = parsed.data;
  const userId = request.user!.id;
  let dates: { snoozedUntil: string | null; resolvedAt: string | null } | null = null;
  if (data.status) {
    try { dates = actionStateDates(data.status, data.snoozedUntil); }
    catch (error) { return validationError(response, (error as Error).message); }
  }
  const outcome = await withTransaction(async (client) => {
    const result = await client.query("UPDATE weekly_reviews SET status=COALESCE($3,status),summary=CASE WHEN $4::boolean THEN $5 ELSE summary END,balance_snapshot_cents=CASE WHEN $6::boolean THEN $7 ELSE balance_snapshot_cents END,snoozed_until=CASE WHEN $3 IS NOT NULL THEN $8 ELSE snoozed_until END,reviewed_at=CASE WHEN $3='resolved' THEN NOW() WHEN $3 IS NOT NULL THEN NULL ELSE reviewed_at END,updated_at=NOW() WHERE id=$1 AND user_id=$2 RETURNING *", [request.params.id, userId, data.status ?? null, "summary" in data, data.summary ?? null, "balanceSnapshot" in data, data.balanceSnapshot == null ? null : moneyToCents(data.balanceSnapshot), dates?.snoozedUntil ?? null]);
    if (!result.rowCount) return { missing: true as const };
    for (const action of data.actions ?? []) {
      if (action.debtId) {
        const debt = await client.query("SELECT id FROM debts WHERE id=$1 AND user_id=$2", [action.debtId, userId]);
        if (!debt.rowCount) return { conflict: "Uma ação referencia uma dívida inexistente." };
      }
      await client.query("INSERT INTO action_items (id,user_id,weekly_review_id,debt_id,title,detail,due_date) VALUES ($1,$2,$3,$4,$5,$6,$7)", [randomUUID(), userId, request.params.id, action.debtId ?? null, action.title, action.detail ?? null, action.dueDate ?? null]);
    }
    await audit(client, userId, "weekly_review_updated", { reviewId: request.params.id, status: data.status, actionCount: data.actions?.length ?? 0 });
    return { row: result.rows[0] };
  });
  if ("missing" in outcome) { response.status(404).json({ message: "Revisão semanal não encontrada." }); return; }
  if ("conflict" in outcome) { response.status(409).json({ message: outcome.conflict }); return; }
  response.json({ weeklyReview: serializeReview(outcome.row) });
}));
